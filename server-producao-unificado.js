...
const setupAPIRoutes = () => {
    ...
    app.get('/api/payments', async (req, res) => {
        try {
            ...
            let query = `
                SELECT p.*, c.name as client_name, pl.name as plan_name 
                FROM payments p 
    ...
    app.post('/api/payments', authenticateToken, async (req, res) => {
        try {
            const { client_id, plan_id, amount, payment_method, notes } = req.body;
            const result = await client.query(
                'INSERT INTO payments (client_id, plan_id, amount, payment_method, notes) VALUES ($1, $2, $3, $4, $5) RETURNING *',
                [client_id, plan_id, amount, payment_method, notes]
            );
            res.json(result.rows[0]);
        } catch (error) {
